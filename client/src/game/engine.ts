...
export const DIFFS: Record<
  Diff,
  { cops: number; spd: number; time: number; spawn: number; max: number }
> = {
...
export const ACHIEVEMENTS = [
  ...
  { id: "coin_hunter", name: "Cazador", desc: "Recoge 50 monedas", icon: "💰" },
  {
    id: "first_blood",
    name: "Primera Sangre",
    desc: "Destruye un policía",
    icon: "💥",
  },
  {
    id: "destroyer",
    name: "Destructor",
    desc: "Destruye 10 policías",
    icon: "🔥",
  },
  {
    id: "drift_king",
    name: "Rey del Drift",
    desc: "Driftea 30 segundos",
    icon: "💨",
  },
  { id: "close_call", name: "Por Poco", desc: "10 roces seguidos", icon: "😰" },
  {
    id: "survivor",
    name: "Superviviente",
    desc: "Sobrevive 2 minutos",
    icon: "⏱️",
  },
  {
    id: "five_stars",
    name: "5 Estrellas",
    desc: "Alcanza nivel máximo",
    icon: "⭐",
  },
  {
    id: "untouchable",
  ...
  },
  {
    ...
    desc: "3 policías explotan a la vez",
    icon: "💣",
  },
  {
    id: "speed_demon",
    name: "Demonio de Velocidad",
    desc: "Vuela con el turbo a tope",
    icon: "🚀",
  },
  {
    id: "demolition",
    name: "Demolición",
    desc: "Destruye 25 policías",
    icon: "🏆",
  },
];
...
export function createGame(
  ...
  return {
    ...
    shake: 0,
    spawnTimer: 0,
    surviveTime: 0, // protección al aparecer
    achievements: savedAch,
    newAchievement: null,
    achieveTimer: 0,
    lostLives: 0,
    wanted: 0,
...
function saveHS(g: Game) {
  localStorage.setItem(
    ...
    Math.max(
      g.score,
      parseInt(localStorage.getItem("escape_hs") || "0")
    ).toString()
  );
}
...
export function simulate(
  ...
  if (drift) {
    ...
    addParticles(
      g,
      p.x - Math.cos(p.angle) * 14,
      p.y - Math.sin(p.angle) * 14,
      1,
  ...
  if (Math.abs(p.speed) > 0.15) {
    ...
    if (keys.has("arrowleft") || keys.has("a"))
      p.angle -= turnRate * spdFactor * dir * dt;
    if (keys.has("arrowright") || keys.has("d"))
      p.angle += turnRate * spdFactor * dir * dt;
  }

  // mover + colisión con edificios (deslizamiento)
  let mvx = Math.cos(p.angle) * p.speed,
    mvy = Math.sin(p.angle) * p.speed;
  let nx = p.x + mvx * dt,
    ny = p.y + mvy * dt;
  ...
  if (p.turbo > 0 && Math.abs(p.speed) > MAX_SPD * 1.5)
    unlock(g, "speed_demon", audio);

  // ---- multiplicador ----
  g.multTimer = Math.max(0, g.multTimer - ms);
  if (g.multTimer <= 0 && g.mult > 1) {
  ...
  g.coins = g.coins.filter(c => {
    ...
    if (p.magnet > 0 && d < 190) {
      ...
      c.x += Math.cos(ang) * 11 * dt;
      c.y += Math.sin(ang) * 11 * dt;
    }
    if (d < (p.magnet > 0 ? 42 : 28)) {
      g.coinsTotal++;
      g.score += Math.floor(15 * g.mult);
      g.mult = Math.min(g.mult + 0.3, 10);
      g.multTimer = 4000;
      audio.play("coin");
  ...
  if (g.powerUps.length < 4 && Math.random() < 0.005 * dt) {
    ...
    g.powerUps.push({
      ...
      y: rp.y,
      id: Date.now(),
      type: isBomb ? "bomb" : t2[Math.floor(Math.random() * 3)],
    });
  }
  ...
  if (g.spawnTimer >= spawnRate && g.cops.length < maxCops) {
    ...
    if (bestD > 600) {
      ...
      g.cops.push({
        x: best.x,
        y: best.y,
        angle: Math.atan2(p.y - best.y, p.x - best.x),
        spd: cfg.spd * (1 + g.stars * 0.08),
        baseSpd: cfg.spd * (1 + g.stars * 0.08),
        id: Date.now(),
        role: roles[g.cops.length % roles.length],
        tier,
        stuck: 0,
      });
    }
  ...
  g.wanted = clamp(
    Math.max(prox, g.stars >= 2 ? 0.35 : 0) * (0.5 + g.stars * 0.12),
    0,
    1
  );
  ...
  for (const cop of g.cops) {
    ...
    if (cop.role === "interceptor") {
      ...
      tx =
        p.x +
        Math.cos(pdir) * ahead +
        Math.cos(pdir + Math.PI / 2) * off * side;
      ty =
        p.y +
        Math.sin(pdir) * ahead +
        Math.sin(pdir + Math.PI / 2) * off * side;
    } else if (cop.role === "blocker") {
      const ahead = 240 + Math.abs(p.speed) * 12;
      tx = p.x + Math.cos(pdir) * ahead;
      ty = p.y + Math.sin(pdir) * ahead;
    }
    ...
    for (const o of g.cops) {
      ...
      if (od > 0 && od < CAR * 2.4) {
        sx += (odx / od) * (CAR * 2.4 - od);
        sy += (ody / od) * (CAR * 2.4 - od);
      }
    }
    if (sx || sy) {
    ...
    const probe = CAR * 2.6;
    const clear = (ang: number) =>
      !pointBlocked(
        solids,
        cop.x + Math.cos(ang) * probe,
        cop.y + Math.sin(ang) * probe,
        4
      ) &&
      cop.x + Math.cos(ang) * probe > 0 &&
      cop.x + Math.cos(ang) * probe < MAP &&
      cop.y + Math.sin(ang) * probe > 0 &&
      cop.y + Math.sin(ang) * probe < MAP;
    if (!clear(target)) {
      let found = false;
    ...
    if (dist < 130 && Math.abs(angDiff(Math.atan2(dy, dx), p.angle)) < 0.7)
      want *= 1.22; // intento de embestida (PIT)
    ...
    if (cop.stuck > 2200) {
      const rp = randomRoadPoint(g.rnd);
      if (Math.hypot(rp.x - p.x, rp.y - p.y) > 500) {
        cop.x = rp.x;
        cop.y = rp.y;
        cop.stuck = 0;
      }
...
export function draw(
  ...
  const inView = (x: number, y: number, m: number) =>
    x - camX > -m && x - camX < W + m && y - camY > -m && y - camY < H + m;
  ...
  for (const lot of g.city.lots) {
    ...
    ctx.strokeStyle =
      lot.ground === "grass"
        ? "rgba(70,170,100,0.22)"
        : "rgba(120,140,200,0.16)";
    ctx.lineWidth = 2;
  ...
  ctx.lineWidth = 1;
  const FINE = 180;
  const fx0 = Math.floor(camX / FINE) * FINE,
    fy0 = Math.floor(camY / FINE) * FINE;
  ...
  const startX = Math.floor(camX / BLOCK) * BLOCK,
    startY = Math.floor(camY / BLOCK) * BLOCK;
  ...
  for (const sk of g.skids) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      sk.x - camX - Math.cos(sk.a) * 6,
      sk.y - camY - Math.sin(sk.a) * 6
    );
    ctx.lineTo(
      sk.x - camX + Math.cos(sk.a) * 6,
      sk.y - camY + Math.sin(sk.a) * 6
    ...
    if (s.kind === "building") {
      ...
      for (let wy = y + 10; wy < y + s.h - 6; wy += 12) {
        for (let wx = x + 6; wx < x + s.w - 6; wx += 11) {
          const lit =
            (Math.floor((wx + sd) * 0.7) ^ Math.floor((wy + sd) * 1.3)) % 5;
      ...
      ctx.beginPath();
      ctx.arc(x + s.w / 2 + 3, y + s.h / 2 + 3, s.w / 2, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = s.color;
      ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.arc(x + s.w / 2 - 3, y + s.h / 2 - 3, s.w / 4, 0, Math.PI * 2);
      ctx.fill();
    } else if (s.kind === "car") {
      ctx.fillStyle = s.color;
      ctx.fillRect(x, y, s.w, s.h);
      ...
      ctx.beginPath();
      ctx.arc(x + s.w / 2, y + s.h / 2, s.w / 4, 0, Math.PI * 2);
      ctx.fill();
    } else if (s.kind === "water") {
      const t = Date.now() * 0.001;
      ctx.fillStyle = "#0e2a4a";
      ctx.fillRect(x, y, s.w, s.h);
      ctx.strokeStyle = "rgba(90,150,220,0.25)";
      ctx.lineWidth = 1.5;
  ...
  for (const t of g.traffic) {
    ...
    ctx.rotate(t.angle);
    drawTrafficCar(ctx, CAR * 0.9, t.color);
    ctx.restore();
  }
  ...
  for (const cop of g.cops) {
    ...
    ctx.save();
    ctx.translate(cop.x - camX, cop.y - camY);
