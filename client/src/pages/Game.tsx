...
function snapshot(g: GameState): Hud {
  return {
    ...
    time: g.time,
    cops: g.cops.length,
    destroyed: g.copsDestroyed,
    shield: g.player.shield > 0,
    turbo: g.player.turbo > 0,
    magnet: g.player.magnet > 0,
    mult: g.mult,
    kmh: Math.round(Math.abs(g.player.speed) * 13),
    newAch: g.newAchievement,
  };
}
  ...
  if (phase === "menu") {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a12] flex flex-col items-center justify-center p-4">
        ...
        <p className="text-[#64748b] text-sm mb-3 tracking-widest">
          CIUDAD 2026 · ESCAPA · DESTRUYE · SOBREVIVE
        </p>
        {hs > 0 && (
          <p className="text-[#00ff88] text-lg mb-3">
            🏆 Récord: {hs.toLocaleString()}
          </p>
        )}
          ...
          <div className="flex gap-2">
            {PAINTS.map((pp, i) => (
              <button
                ...
                style={{
                  borderColor: pp.body,
                  boxShadow: paintIdx === i ? `0 0 14px ${pp.glow}` : "none",
                }}
              >
                <span
                  className="block w-8 h-4 rounded"
                  style={{
                    background: `linear-gradient(90deg, ${pp.body2}, ${pp.body})`,
                  }}
                />
              </button>
            ))}
          </div>
          <p className="mt-1 text-sm font-bold" style={{ color: paint.body }}>
            {paint.name}
          </p>
        </div>
          ...
          {(["normal", "hard", "impossible"] as Diff[]).map(d => (
            <button
              ...
              className={`p-3 rounded-lg border-2 transition-all ${
                diff === d
                  ? "border-[#ecc94b] bg-[#ecc94b]/20 scale-105"
                  : "border-[#4a5568] bg-[#161622]"
              }`}
            >
              <span
                className={`font-bold ${d === "normal" ? "text-green-400" : d === "hard" ? "text-yellow-400" : "text-red-400"}`}
              >
                {d === "normal"
                  ? "⭐ NORMAL · 4 patrullas"
                  : d === "hard"
                    ? "⭐⭐ DIFÍCIL · 6 patrullas"
                    : "⭐⭐⭐ IMPOSIBLE · 8 patrullas"}
              </span>
            </button>
          ))}
        </div>
        ...
        <div className="mt-4 text-[#475569] text-xs space-y-1 text-center">
          <p>
            <span className="text-[#ecc94b]">↑↓←→ / WASD</span> Conducir ·{" "}
          ...
          <p className="text-[#e53e3e]">
            ¡Esquiva edificios y haz chocar a las patrullas!
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#0a0a12] flex flex-col items-center justify-center p-2">
      ...
      <div className="flex items-center justify-between w-full max-w-[960px] mb-2 px-2 gap-2 flex-wrap">
        ...
        <div
          className={`bg-black/70 px-4 py-1 rounded-lg border ${hud.stars >= 3 ? "border-[#e53e3e] animate-pulse" : "border-[#e53e3e]/50"}`}
        >
          <span className="text-xl">
            {"⭐".repeat(hud.stars)}
            {"☆".repeat(5 - hud.stars)}
        ...
        <div className="bg-black/70 px-4 py-1 rounded-lg border border-[#00ffff]">
          ...
          <p className="text-[#00ffff] font-bold text-center">
            {hud.kmh} <span className="text-[9px]">km/h</span>
          </p>
        </div>
        ...
        <div className="bg-black/70 px-5 py-1 rounded-lg border border-[#ecc94b]">
          ...
          <p
            className={`text-xl font-bold ${hud.time < 15000 ? "text-[#e53e3e] animate-pulse" : "text-[#ecc94b]"}`}
          >
            {fmt(hud.time)}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {hud.shield && (
            <div className="bg-[#00ff88]/30 px-2 py-1 rounded">🛡️</div>
          )}
          {hud.turbo && (
            <div className="bg-[#00ffff]/30 px-2 py-1 rounded">⚡</div>
          )}
          {hud.magnet && (
            <div className="bg-[#ff00ff]/30 px-2 py-1 rounded">🧲</div>
          )}
          <div className="bg-black/70 px-2 py-1 rounded-lg">
            <span className="text-[#e53e3e]">🚔 {hud.cops}</span>
          </div>
          <div className="bg-black/70 px-2 py-1 rounded-lg">
            <span className="text-[#ff6600]">💥 {hud.destroyed}</span>
          </div>
          <button
            onClick={() => setMuted(audio.toggleMute())}
            className="bg-black/70 px-2 py-1 rounded-lg"
          >
            {muted ? "🔇" : "🔊"}
          </button>
        </div>
      ...
      <div className="relative border-4 border-[#1f2937] rounded-lg overflow-hidden shadow-2xl shadow-[#00f5ff]/10">
        ...
        {phase === "paused" && (
          <div className="absolute inset-0 bg-black/85 flex flex-col items-center justify-center">
            ...
            <button
              onClick={togglePause}
              className="px-8 py-3 bg-[#ecc94b] text-[#1a1a2e] font-bold rounded-lg mb-3"
            >
              CONTINUAR
            </button>
            <button
              onClick={goMenu}
              className="px-8 py-3 border-2 border-[#e53e3e] text-[#e53e3e] rounded-lg"
            >
              SALIR
            </button>
          </div>
        ...
        {phase === "won" && gameRef.current && (
          <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center">
            ...
            <p className="text-[#ecc94b] text-5xl font-bold mb-2">
              {hud.score.toLocaleString()}
            </p>
            <p className="text-[#64748b] mb-4">
              💰 {hud.coins} · 💥 {hud.destroyed} destruidos
            </p>
            <button
              onClick={() => startGame(diff)}
              className="px-8 py-3 bg-[#00ff88] text-[#1a1a2e] font-bold rounded-lg mb-3"
            >
              JUGAR DE NUEVO
            </button>
            <button
              onClick={goMenu}
              className="px-8 py-3 border-2 border-[#ecc94b] text-[#ecc94b] rounded-lg"
            >
              MENÚ
            </button>
          </div>
        ...
        {phase === "over" && (
          <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center">
            ...
            <p className="text-[#64748b] mb-4">
              💰 {hud.coins} · 💥 {hud.destroyed} · ⏱ {fmt(hud.time)}
            </p>
            <button
              onClick={() => startGame(diff)}
              className="px-8 py-3 bg-[#ecc94b] text-[#1a1a2e] font-bold rounded-lg mb-3"
        ...
        {hud.mult > 1 && phase === "playing" && (
          <div className="absolute top-2 right-3 pointer-events-none">
            <span
              className={`font-black text-3xl ${hud.mult >= 5 ? "text-[#ff00ff]" : "text-[#00ff88]"}`}
        ...
        {hud.newAch && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/85 border-2 border-[#ecc94b] rounded-lg px-5 py-2 text-center pointer-events-none">
            ...
            <p className="text-[#ecc94b]">
              {hud.newAch.icon} {hud.newAch.name}
            </p>
          </div>
        )}
      </div>
          ...
          <canvas
            ...
            ref={mini => {
              ...
              for (const pu of g.powerUps) {
                ctx.beginPath();
                ctx.arc(pu.x * sc, pu.y * sc, 2, 0, Math.PI * 2);
                ctx.fill();
              }
              // patrullas
              ...
              for (const c of g.cops) {
                ctx.beginPath();
                ctx.arc(c.x * sc, c.y * sc, 2.4, 0, Math.PI * 2);
                ctx.fill();
              }
              // jugador
              ...
              ctx.lineTo(
                g.player.x * sc + Math.cos(g.player.angle) * 8,
                g.player.y * sc + Math.sin(g.player.angle) * 8
              );
              ctx.stroke();
            }}
          />
      ...
      <div className="mt-1 text-[#475569] text-xs">
        ...
        <span className="text-[#00ffff]">ESPACIO</span> derrape ·{" "}
        <span className="text-[#ecc94b]">P</span> pausa ·{" "}
        <span className="text-[#e53e3e]">¡Haz chocar a las patrullas!</span>
      </div>
    </div>
  );
}
